  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-white border border-slate-200 rounded-3xl max-w-lg w-full p-5 sm:p-6 shadow-2xl text-slate-900 animate-in zoom-in-95 duration-200 space-y-4 max-h-[92vh] overflow-y-auto">
        ...
        {/* 🏪 Seller Location & Real-Time Swiggy/Zomato Style ETA Card */}
        <div className="bg-gradient-to-br from-emerald-50/90 to-teal-50/50 p-4 rounded-2xl border border-emerald-200/80 space-y-2.5">
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-black text-slate-900">{estimate.sellerName}</span>
                  <span className="inline-flex items-center gap-0.5 bg-emerald-100 text-emerald-800 text-[10px] font-black px-1.5 py-0.2 rounded">
                    <Star className="w-2.5 h-2.5 fill-emerald-600 text-emerald-600" />
                    <span>{estimate.sellerRating}</span>
                  </span>
                </div>
                <div className="text-[11px] text-slate-600 flex items-center gap-1 mt-0.5">
                  <MapPin className="w-3 h-3 text-slate-400 shrink-0" />
                  <span>{estimate.sellerAddress}</span>
                </div>
              </div>
            </div>

            <div className="text-right shrink-0">
              <span className={`text-[10px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider ${estimate.badgeColor}`}>
                {estimate.badge}
              </span>
              <div className="text-sm font-black text-emerald-900 mt-1">
                ⚡ {estimate.formattedEta}
              </div>
              <div className="text-[10px] text-slate-500 font-bold">
                {estimate.formattedDist} away
              </div>
            </div>
          </div>

          {/* Delivery timeline calculation breakdown */}
          <div className="pt-2 border-t border-emerald-200/60 grid grid-cols-3 gap-2 text-center text-[10.5px]">
            <div className="bg-white/80 p-1.5 rounded-lg border border-emerald-100">
              <div className="text-slate-400 font-bold">1. Order Picking</div>
              <div className="font-black text-slate-800">~2 Mins</div>
            </div>
            <div className="bg-white/80 p-1.5 rounded-lg border border-emerald-100">
              <div className="text-slate-400 font-bold">2. Road Transit</div>
              <div className="font-black text-slate-800">{estimate.formattedDist}</div>
            </div>
            <div className="bg-white/80 p-1.5 rounded-lg border border-emerald-100">
              <div className="text-slate-400 font-bold">3. Floor Drop</div>
              <div className="font-black text-emerald-700 font-bold">Arriving {estimate.arrivalTimeFormatted}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200/80 text-xs">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-[10px] uppercase font-bold text-slate-400">Delivery SLA</div>
              <div className="font-black text-emerald-800">{estimate.formattedEta} to Jobsite</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="truncate">
              <div className="text-[10px] uppercase font-bold text-slate-400">Warehouse Bin</div>
              <div className="font-bold text-slate-800 truncate">{product.binLocation}</div>
            </div>
          </div>
        </div>
        ...
        <div className={`p-3.5 rounded-2xl border ${isGst ? 'bg-sky-50/70 border-sky-200' : 'bg-slate-50 border-slate-200'} space-y-2 text-xs`}>
          ...
          {isGst ? (
            ...
            <p className="text-slate-500 text-[11px]">
              This product is sold by a local unregistered vendor. No B2B GST tax credit (ITC) invoice will be generated.
            </p>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100">
          <div>
            ...
            <div className="text-[10px] text-emerald-700 font-bold">{product.stockCount} available at {estimate.sellerLocality}</div>
          </div>
          <button
            onClick={() => {
