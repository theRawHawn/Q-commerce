...
export const createRiderIcon = () => {
  return L.divIcon({
    ...
    html: `
      <div style="position: relative; display: flex; flex-direction: column; align-items: center; cursor: pointer;">
        <!-- Courier Avatar Disc -->
        <div style="width: 42px; height: 42px; background: linear-gradient(135deg, #059669 0%, #047857 100%); border-radius: 9999px; display: flex; align-items: center; justify-content: center; box-shadow: 0 8px 20px rgba(5,150,105,0.45); border: 3px solid #ffffff; position: relative; transform: translate3d(0,0,0);">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#ffffff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
            <circle cx="18.5" cy="17.5" r="3.5"/>
            <circle cx="5.5" cy="17.5" r="3.5"/>
            <path d="M15 6a1 1 0 1 0 0-2 1 1 0 0 0 0 2zm-3 11.5V14l-3-3 4-3 2 3h4"/>
          </svg>
          <!-- Battery/Express Badge -->
          <div style="position: absolute; bottom: -2px; right: -2px; width: 15px; height: 15px; background: #f59e0b; border-radius: 9999px; border: 2px solid white; display: flex; align-items: center; justify-content: center; font-size: 8px; font-weight: 900; color: #78350f;">
            ⚡
          </div>
        </div>

        <!-- Motion Ground Wave -->
        <div style="position: absolute; bottom: -6px; width: 32px; height: 8px; background: radial-gradient(ellipse at center, rgba(5,150,105,0.4) 0%, rgba(5,150,105,0) 75%); border-radius: 9999px;"></div>
      </div>
    `,
    iconSize: [44, 52],
    iconAnchor: [22, 26],
...
export const OpenStreetMap: React.FC<OpenStreetMapProps> = ({
  ...
  useEffect(() => {
    ...
    if (riderMarkerRef.current) {
      ...
      riderMarkerRef.current = L.marker(currentRiderPos, { icon: createRiderIcon() })
        .bindTooltip(`<strong>${riderName}</strong><br>⚡ 12-Min Express EV Courier`, {
          permanent: true,
          direction: 'top',
          offset: [0, -22],
  ...
  }, [isRouteMode, hubLocation, destinationLocation, riderLocation, riderName]);
